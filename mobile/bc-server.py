#!/usr/bin/env python3
"""Betta Mobile Server — Token auth + SSE"""
import os
import json
import time
import secrets
...
def task_detail(task_id):
    ...
    ''', (task_id,)).fetchall()
    conn.close()
    return jsonify({'task': row_to_dict(task), 'messages': [row_to_dict(m) for m in messages]})
...
def heartbeat():
    def stream():
        ...
        while True:
            try:
                conn   = get_db()
                result = conn.execute('SELECT MAX(updated_at) as latest FROM tasks').fetchone()
                conn.close()
...
if __name__ == '__main__':
    print(f"\n{'─'*52}")
    print(f"  Better Control Mobile  ·  Task Coordination")
    print(f"{'─'*52}")
